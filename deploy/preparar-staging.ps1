...
Write-Host "==> 1/5 Instalando dependencias de produccion del backend..." -ForegroundColor Cyan
Push-Location $backend
npm install --omit=dev
Write-Host "==> 2/5 Generando certificado HTTPS..." -ForegroundColor Cyan
node generar-certificado.js
Pop-Location

Write-Host "==> 3/5 Compilando el frontend..." -ForegroundColor Cyan
Push-Location $frontend
npm install
npm run build
Pop-Location

Write-Host "==> 4/5 Limpiando staging anterior..." -ForegroundColor Cyan
if (Test-Path $staging) { Remove-Item -Recurse -Force $staging }
New-Item -ItemType Directory -Force -Path $staging | Out-Null
New-Item -ItemType Directory -Force -Path (Join-Path $staging "parkpro-frontend") | Out-Null

Write-Host "==> 5/5 Copiando archivos a staging..." -ForegroundColor Cyan
# Backend completo (incluye node_modules y certs), excluyendo sesiones y basura
robocopy $backend (Join-Path $staging "parkpro-backend") /E `
  /XD ".baileys_auth" ".wwebjs_auth" ".wwebjs_cache" ".git" | Out-Null
# Frontend compilado
robocopy (Join-Path $frontend "dist") (Join-Path $staging "parkpro-frontend\dist") /E | Out-Null
